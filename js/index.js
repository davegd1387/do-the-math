...
function builddivLine(ctr, divLineExpense, divLinePrice) {
  ...
  var divLine =
    ...
    '   tabindex="1" ';
  divLine +=
    'size="8" class="text" onfocus=ChangeFocusColor(this) onblur=ChangeBlurColor(this)></td>';
  divLine +=
    "<td id=" +
    divLinePriceID +
    " class=price align=right>" +
    divLinePrice +
    "</td></tr>";
  return divLine;
}
...
function addNewLine() {
  var divHTML =
    '<table><tr><td align="left" class="text2">Line Cost:</td><td align="left" id=priceTitle1';
  divHTML += ' class ="text2">Line Price:</td></tr>';
  for (var h = 0; h < lineCtr; h++) {
    divHTML += builddivLine(
      h,
      document.getElementById("expense" + h).value,
      document.getElementById("price" + h).innerHTML
    );
  }
  divHTML += builddivLine(lineCtr, 0, "--------------------");
  divHTML +=
    '<tr><td></td></tr><tr><td align="left" class=price><b>Total Cost:</b></td>';
  divHTML += "<td id=totalExpense class=price>--------------------</td>";
  divHTML += '<td align="left" class=price><b>GP per Covering:</b></td>';
  divHTML += "<td id=GPW class=price>--------------------</td>";
...
function dotheMATH() {
  ...
  var totalPrice = parseFloat(
    document.getElementById("totalPrice").value.replace(/[\$\,]/g, "")
  );
  var totalWindows = document.getElementById("totalWindows").value;
  ...
  if ((totalPrice <= 0) | isNaN(totalPrice)) {
    ...
    for (var j = 0; j < lineCtr; j++) {
      expense[j] = parseFloat(
        document.getElementById("expense" + j).value.replace(/[\$\,]/g, "")
      );
      if (isNaN(expense[j])) {
    ...
    GP = totalPrice - totalExpense;
    //GPM = (totalPrice - totalExpense) / totalPrice * 100
    GPM = (GP / totalPrice) * 100;
    GPW = GP / totalWindows;
    document.getElementById("GPM").innerHTML = GPM.toFixed(3) + " %";
    document.getElementById("GPW").innerHTML = " $" + GPW.toFixed(2);
    ...
    if (totPrice < totalPrice) {
      ...
      document.getElementById("price" + (lineCtr - 1)).innerHTML =
        thisPrice.toFixed(2);
    }
    if (totPrice > totalPrice) {
...
//   window.open(
//     "http://bfast.budgetblinds.com/",
//     "_blank",
//     "toolbar=yes, location=yes, directories=no, status=no, menubar=yes, scrollbars=yes, resizable=yes, copyhistory=yes, width=875, height=700, left=500, top=0"
//   );
// }
...
//   document.getElementById("newLine").addEventListener("onclick", addNewLine);
//   document.getElementById("calculate").addEventListener("onclick", dotheMATH());
//   document
//     .getElementById("refreshbtn")
//     .addEventListener("onclick", refreshIT());
//   document.getElementById("clear").addEventListener("onclick", clearIT());
// };
// window.onload = function () {
//   "use strict";
...
//   U.addEvent(U.$("totalPrice"), "focus", function () {
//     totalPrice.style.background = "white";
//   });
//   U.addEvent(U.$("totalPrice"), "blur", function () {
//     totalPrice.style.background = "#D7AF28";
//   });
//   addNewLine();
// };
